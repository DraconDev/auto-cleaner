    ...
    public async commitChanges(cwd: string, message: string): Promise<boolean> {
        try {
            ...
            vscode.window.showInformationMessage(
                `auto cleaner pro: Created backup commit: "${message}"`
            );
            return true;
        } catch (error) {
            Logger.error("[GitManager] Commit failed", error);
            vscode.window.showErrorMessage(
                `auto cleaner pro: Failed to create backup commit. Check Output channel.`
            );
            return false;
        }
    }

    public async pushChanges(cwd: string): Promise<boolean> {
        try {
            Logger.log(`[GitManager] Pushing changes in ${cwd}`);
            await this.runCommand("git push", cwd);
            Logger.log("[GitManager] Push successful.");
            return true;
        } catch (error) {
            Logger.error("[GitManager] Push failed", error);
            vscode.window.showErrorMessage(
                `auto cleaner pro: Failed to push changes. Check Output channel.`
            );
            return false;
        }
    }
