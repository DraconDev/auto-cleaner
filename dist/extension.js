...
async function performClean() {
    try {
        ...
        if (!commitSuccess) {
            Logger_1.Logger.log("[Extension] User cancelled or git commit failed");
            return;
        }
        const action = await vscode.window.showWarningMessage("This will remove all detected unused code across all languages. Use Ctrl+Z to undo.", { modal: true }, "Clean All", "Cancel");
        if (action !== "Clean All") {
            return;
        }
            ...
            if (analyzer) {
                try {
                    ...
                    if (result.errors.length > 0) {
                        errors.push(...result.errors.map((e) => `${analyzerName}: ${e.error}`));
                    }
                }
                catch (error) {
                    Logger_1.Logger.error(`[Extension] Error cleaning ${analyzerName}`, error);
